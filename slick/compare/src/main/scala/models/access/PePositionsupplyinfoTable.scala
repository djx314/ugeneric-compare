...
trait PePositionsupplyinfoTable {
  ...
  case class PePositionsupplyinfoRow(positionsupplyinfoid: Int = 0, username: Option[String], positionid: Option[Int] = Some(0), subcompanyid: Option[Int] = Some(0), workplaceid: Option[Int] = Some(0), checkstatus: Option[Int] = Some(0), checkdate: Option[java.sql.Timestamp], supplydate: Option[java.sql.Timestamp])
  ...
  class PePositionsupplyinfo(_tableTag: Tag) extends profile.api.Table[PePositionsupplyinfoRow](_tableTag, "PE_PositionSupplyInfo") {
    ...
    val index1 = index("PE_POSITIONSUPPLYINFO_INTERVIEWEEID", username)
    /** Index over (positionid) (database name PE_POSITIONSUPPLYINFO_POSITIONID) */
    val index2 = index("PE_POSITIONSUPPLYINFO_POSITIONID", positionid)
    /** Index over (subcompanyid) (database name PE_POSITIONSUPPLYINFO_SUBCOMPANYID) */
    val index3 = index("PE_POSITIONSUPPLYINFO_SUBCOMPANYID", subcompanyid)
    /** Index over (workplaceid) (database name PE_POSITIONSUPPLYINFO_WORKPLACEID) */
    val index4 = index("PE_POSITIONSUPPLYINFO_WORKPLACEID", workplaceid)
  }
