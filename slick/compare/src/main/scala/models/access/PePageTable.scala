...
trait PePageTable {
  ...
  case class PePageRow(id: Int, classname: Option[String], pagename: String, pageintro: String, pageurl: Option[String], pagefilename: Option[String], pagetype: Option[Short] = Some(0), pagecontent: Option[String], arrgroupid: Option[String])
  ...
  class PePage(_tableTag: Tag) extends profile.api.Table[PePageRow](_tableTag, "PE_Page") {
    ...
    val index1 = index("PE_PAGE_ARRGROUPID", arrgroupid)
    /** Index over (classname) (database name PE_PAGE_GROUPID) */
    val index2 = index("PE_PAGE_GROUPID", classname)
  }
