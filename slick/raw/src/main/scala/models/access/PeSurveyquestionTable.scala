...
trait PeSurveyquestionTable {
  ...
  case class PeSurveyquestionRow(questionid: Int, surveyid: Option[Int] = Some(0), questioncontent: Option[String], questiontype: Option[Short] = Some(1), inputtype: Option[Short] = Some(0), notempty: Option[Int] = Some(0), dataright: Option[Int] = Some(0), orderid: Option[Int] = Some(0), contentlength: Option[Int] = Some(0))
  ...
  class PeSurveyquestion(_tableTag: Tag) extends profile.api.Table[PeSurveyquestionRow](_tableTag, "PE_SurveyQuestion") {
    def * = (questionid :: surveyid :: questioncontent :: questiontype :: inputtype :: notempty :: dataright :: orderid :: contentlength :: HNil).mapTo[PeSurveyquestionRow]
    ...
    val index1 = index("PE_SURVEYQUESTION_I_ID", surveyid :: HNil)
    /** Index over (orderid) (database name PE_SURVEYQUESTION_ORDERID) */
    val index2 = index("PE_SURVEYQUESTION_ORDERID", orderid :: HNil)
  }
