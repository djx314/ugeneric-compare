...
// AUTO-GENERATED Slick data model for table PeSpacemusic
trait PeSpacemusicTable {
  ...
  /** Entity class storing rows of table PeSpacemusic
  ...
  case class PeSpacemusicRow(id: Int = 0, userid: Int = 0, blogid: Option[Int] = Some(0), title: String, content: Option[String], datetime: java.sql.Timestamp, hits: Option[Int] = Some(0), plnum: Option[Int] = Some(0))
  /** GetResult implicit for fetching PeSpacemusicRow objects using plain SQL queries */
  ...
  class PeSpacemusic(_tableTag: Tag) extends profile.api.Table[PeSpacemusicRow](_tableTag, "PE_SpaceMusic") {
    def * = (id :: userid :: blogid :: title :: content :: datetime :: hits :: plnum :: HNil).mapTo[PeSpacemusicRow]
    ...
    /** Index over (plnum) (database name PE_SPACEMUSIC_PLNUM) */
    val index1 = index("PE_SPACEMUSIC_PLNUM", plnum :: HNil)
    /** Index over (userid) (database name PE_SPACEMUSIC_USERID) */
    val index2 = index("PE_SPACEMUSIC_USERID", userid :: HNil)
    /** Index over (blogid) (database name PE_SPACEMUSIC_USERID1) */
    val index3 = index("PE_SPACEMUSIC_USERID1", blogid :: HNil)
  }
  /** Collection-like TableQuery object for table PeSpacemusic */
  lazy val PeSpacemusic = new TableQuery(tag => new PeSpacemusic(tag))
}
