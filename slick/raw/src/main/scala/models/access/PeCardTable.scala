...
trait PeCardTable {
  ...
  case class PeCardRow(cardid: Int, cardtype: Option[Int] = Some(0), productid: Option[Int] = Some(0), cardnum: Option[String], password: Option[String], agentname: Option[String], money: Option[scala.math.BigDecimal] = Some(scala.math.BigDecimal("0")), validnum: Option[Int] = Some(0), validunit: Option[Int] = Some(0), enddate: Option[java.sql.Timestamp], username: Option[String], usetime: Option[java.sql.Timestamp], createtime: Option[java.sql.Timestamp], orderformitemid: Option[Int] = Some(0))
  ...
  class PeCard(_tableTag: Tag) extends profile.api.Table[PeCardRow](_tableTag, "PE_Card") {
    def * = (cardid :: cardtype :: productid :: cardnum :: password :: agentname :: money :: validnum :: validunit :: enddate :: username :: usetime :: createtime :: orderformitemid :: HNil).mapTo[PeCardRow]
    ...
    val index1 = index("PE_CARD_CARDNUM", cardnum :: HNil)
    /** Index over (password) (database name PE_CARD_CARDPASSWORD) */
    val index2 = index("PE_CARD_CARDPASSWORD", password :: HNil)
    /** Index over (cardtype) (database name PE_CARD_CARDTYPE) */
    val index3 = index("PE_CARD_CARDTYPE", cardtype :: HNil)
    /** Index over (orderformitemid) (database name PE_CARD_ORDERFORMITEMID) */
    val index4 = index("PE_CARD_ORDERFORMITEMID", orderformitemid :: HNil)
    /** Index over (productid) (database name PE_CARD_PRODUCTID) */
    val index5 = index("PE_CARD_PRODUCTID", productid :: HNil)
    /** Index over (validnum) (database name PE_CARD_VALIDNUM) */
    val index6 = index("PE_CARD_VALIDNUM", validnum :: HNil)
  }
