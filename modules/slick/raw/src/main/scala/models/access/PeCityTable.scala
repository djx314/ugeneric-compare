...
trait PeCityTable {
  ...
  class PeCity(_tableTag: Tag) extends profile.api.Table[PeCityRow](_tableTag, "PE_City") {
    def * = (areaid :: country :: province :: city :: area :: postcode :: areacode :: HNil).mapTo[PeCityRow]
    ...
    val index1 = index("PE_CITY_AREACODE", areacode :: HNil)

    /** Index over (postcode) (database name PE_CITY_POSTCODE) */
    val index2 = index("PE_CITY_POSTCODE", postcode :: HNil)
  }
