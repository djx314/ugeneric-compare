...
trait PeSurveyanswerTable {
  ...
  case class PeSurveyanswerRow(
    answerid: Int,
    questionid: Option[Int] = Some(0),
    voteamount: Option[Int] = Some(0),
    orderid: Option[Int] = Some(0),
    answercontent: Option[String]
  )
  ...
  class PeSurveyanswer(_tableTag: Tag) extends profile.api.Table[PeSurveyanswerRow](_tableTag, "PE_SurveyAnswer") {
    def * = (answerid :: questionid :: voteamount :: orderid :: answercontent :: HNil).mapTo[PeSurveyanswerRow]
    ...
    val index1 = index("PE_SURVEYANSWER_ITEM_ID", questionid :: HNil)

    /** Index over (orderid) (database name PE_SURVEYANSWER_ORDERID) */
    val index2 = index("PE_SURVEYANSWER_ORDERID", orderid :: HNil)
  }
