...
trait PeSpacevisitorTable {
  ...
  case class PeSpacevisitorRow(id: Int, blogid: Option[Int] = Some(0), userid: Int = 0, username: Option[String], datetime: java.sql.Timestamp, num: Option[Int] = Some(0))
  ...
  class PeSpacevisitor(_tableTag: Tag) extends profile.api.Table[PeSpacevisitorRow](_tableTag, "PE_SpaceVisitor") {
    def * = (id :: blogid :: userid :: username :: datetime :: num :: HNil).mapTo[PeSpacevisitorRow]
    ...
    val index1 = index("PE_SPACEVISITOR_BLOGID", blogid :: HNil)
    /** Index over (num) (database name PE_SPACEVISITOR_NUM) */
    val index2 = index("PE_SPACEVISITOR_NUM", num :: HNil)
    /** Index over (userid) (database name PE_SPACEVISITOR_USERID) */
    val index3 = index("PE_SPACEVISITOR_USERID", userid :: HNil)
    /** Index over (username) (database name PE_SPACEVISITOR_USERID1) */
    val index4 = index("PE_SPACEVISITOR_USERID1", username :: HNil)
  }
