...
trait PeTrademarkTable {
  ...
  case class PeTrademarkRow(
    trademarkid: Int,
    channelid: Option[Int] = Some(0),
    producerid: Option[Int] = Some(0),
    trademarkname: Option[String],
    trademarkphoto: Option[String],
    trademarkintro: Option[String],
    trademarktype: Option[Short] = Some(0),
  ...
  class PeTrademark(_tableTag: Tag) extends profile.api.Table[PeTrademarkRow](_tableTag, "PE_Trademark") {
    def * =
      (trademarkid :: channelid :: producerid :: trademarkname :: trademarkphoto :: trademarkintro :: trademarktype :: passed :: ontop :: iselite :: hits :: HNil)
        .mapTo[PeTrademarkRow]
    ...
    val index1 = index("PE_TRADEMARK_CHANNELID", channelid :: HNil)

    /** Index over (producerid) (database name PE_TRADEMARK_PRODUCERID) */
    val index2 = index("PE_TRADEMARK_PRODUCERID", producerid :: HNil)
  }
