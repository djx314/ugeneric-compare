...
trait PeProducerTable {
  ...
  case class PeProducerRow(
    producerid: Int,
    channelid: Option[Int] = Some(0),
    producername: Option[String],
    producershortname: Option[String],
    producerphoto: Option[String],
    ...
    producertype: Option[Short] = Some(0)
  )
  ...
  class PeProducer(_tableTag: Tag) extends profile.api.Table[PeProducerRow](_tableTag, "PE_Producer") {
    def * =
      (producerid :: channelid :: producername :: producershortname :: producerphoto :: birthday :: address :: postcode :: phone :: fax :: email :: homepage :: producerintro :: lastusetime :: passed :: ontop :: iselite :: hits :: producertype :: HNil)
        .mapTo[PeProducerRow]
    ...
    val index1 = index("PE_PRODUCER_CHANNELID", channelid :: HNil)

    /** Index over (postcode) (database name PE_PRODUCER_POSTCODE) */
    val index2 = index("PE_PRODUCER_POSTCODE", postcode :: HNil)
  }
