...
trait PePresentprojectTable {
  ...
  case class PePresentprojectRow(
    projectid: Int,
    projectname: Option[String],
    begindate: Option[java.sql.Timestamp],
    enddate: Option[java.sql.Timestamp],
    minmoney: Option[scala.math.BigDecimal] = Some(scala.math.BigDecimal("0")),
    maxmoney: Option[scala.math.BigDecimal] = Some(scala.math.BigDecimal("0")),
    presentcontent: Option[String],
    price: Option[scala.math.BigDecimal] = Some(scala.math.BigDecimal("0")),
    presentid: Option[String],
    cash: Option[scala.math.BigDecimal] = Some(scala.math.BigDecimal("0")),
  ...
  class PePresentproject(_tableTag: Tag) extends profile.api.Table[PePresentprojectRow](_tableTag, "PE_PresentProject") {
    def * =
      (projectid :: projectname :: begindate :: enddate :: minmoney :: maxmoney :: presentcontent :: price :: presentid :: cash :: presentexp :: presentpoint :: HNil)
        .mapTo[PePresentprojectRow]
    ...
    val index1 = index("PE_PRESENTPROJECT_BEGINDATE", begindate :: HNil)

    /** Index over (enddate) (database name PE_PRESENTPROJECT_ENDDATE) */
    val index2 = index("PE_PRESENTPROJECT_ENDDATE", enddate :: HNil)

    /** Index over (maxmoney) (database name PE_PRESENTPROJECT_MAXMONEY) */
    val index3 = index("PE_PRESENTPROJECT_MAXMONEY", maxmoney :: HNil)

    /** Index over (minmoney) (database name PE_PRESENTPROJECT_MINMONEY) */
    val index4 = index("PE_PRESENTPROJECT_MINMONEY", minmoney :: HNil)
  }
