...
trait PeHistrolynewsTable {
  ...
  class PeHistrolynews(_tableTag: Tag) extends profile.api.Table[PeHistrolynewsRow](_tableTag, "PE_HistrolyNews") {
    def * = (histrolynewsid :: itemid :: channelid :: classid :: articleid :: title :: newscollecdate :: newsurl :: result :: HNil).mapTo[PeHistrolynewsRow]
    ...
    val index1 = index("PE_HISTROLYNEWS_ARTICLEID", articleid :: HNil)

    /** Index over (channelid) (database name PE_HISTROLYNEWS_CHANNELID) */
    val index2 = index("PE_HISTROLYNEWS_CHANNELID", channelid :: HNil)

    /** Index over (classid) (database name PE_HISTROLYNEWS_CLASSID) */
    val index3 = index("PE_HISTROLYNEWS_CLASSID", classid :: HNil)

    /** Index over (itemid) (database name PE_HISTROLYNEWS_ITEMID1) */
    val index4 = index("PE_HISTROLYNEWS_ITEMID1", itemid :: HNil)
  }
