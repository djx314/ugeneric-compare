...
trait PeInfosTable {
  ...
  class PeInfos(_tableTag: Tag) extends profile.api.Table[PeInfosRow](_tableTag, "PE_InfoS") {
    def * = (infoid :: itemid :: specialid :: moduletype :: HNil).mapTo[PeInfosRow]
    ...
    val index1 = index("PE_INFOS_ITEMID", itemid :: HNil)

    /** Index over (specialid) (database name PE_INFOS_SPECIALID) */
    val index2 = index("PE_INFOS_SPECIALID", specialid :: HNil)
  }
