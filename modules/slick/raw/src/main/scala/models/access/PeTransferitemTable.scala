...
trait PeTransferitemTable {
  ...
  case class PeTransferitemRow(
    itemid: Int,
    orderformid: Option[Int] = Some(0),
    ownerusername: Option[String],
    targetusername: Option[String],
    poundage: Option[scala.math.BigDecimal] = Some(scala.math.BigDecimal("0")),
    payerusername: Option[String],
  ...
  class PeTransferitem(_tableTag: Tag) extends profile.api.Table[PeTransferitemRow](_tableTag, "PE_TransferItem") {
    def * = (itemid :: orderformid :: ownerusername :: targetusername :: poundage :: payerusername :: remark :: dateandtime :: inputer :: HNil).mapTo[PeTransferitemRow]
    ...
    val index1 = index("PE_TRANSFERITEM_ORDERFORMID", orderformid :: HNil)
  }
