...
trait PeSpacecommentTable {
  ...
  case class PeSpacecommentRow(id: Int, blogid: Option[Int] = Some(0), itemid: Option[Int] = Some(0), `type`: Option[Short] = Some(0), uname: Option[String], title: String, content: Option[String], datetime: java.sql.Timestamp)
  ...
  class PeSpacecomment(_tableTag: Tag) extends profile.api.Table[PeSpacecommentRow](_tableTag, "PE_SpaceComment") {
    def * = (id :: blogid :: itemid :: `type` :: uname :: title :: content :: datetime :: HNil).mapTo[PeSpacecommentRow]
    ...
    val index1 = index("PE_SPACECOMMENT_ITEMID", blogid :: HNil)
    /** Index over (`type`) (database name PE_SPACECOMMENT_TYPEID) */
    val index2 = index("PE_SPACECOMMENT_TYPEID", `type` :: HNil)
    /** Index over (itemid) (database name PE_SPACECOMMENT_USERID1) */
    val index3 = index("PE_SPACECOMMENT_USERID1", itemid :: HNil)
  }
