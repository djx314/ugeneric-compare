...
trait PePositionTable {
  ...
  case class PePositionRow(positionid: Int = 0, jobcategoryid: Option[Int] = Some(0), subcompanyid: Option[Int] = Some(0), workplaceid: Option[Int] = Some(0), releasedate: Option[java.sql.Timestamp], positionname: Option[String], positionnum: Option[Int] = Some(0), validdate: Option[Int] = Some(0), positionkeyword: Option[String], positiondescription: Option[String], dutyrequest: Option[String], positionstatus: Option[Int] = Some(0), subcompanyname: Option[String], urgent: Option[Int] = Some(0))
  ...
  class PePosition(_tableTag: Tag) extends profile.api.Table[PePositionRow](_tableTag, "PE_Position") {
    def * = (positionid :: jobcategoryid :: subcompanyid :: workplaceid :: releasedate :: positionname :: positionnum :: validdate :: positionkeyword :: positiondescription :: dutyrequest :: positionstatus :: subcompanyname :: urgent :: HNil).mapTo[PePositionRow]
    ...
    val index1 = index("PE_POSITION_JOBCATEGORYID", jobcategoryid :: HNil)
    /** Index over (positionnum) (database name PE_POSITION_POSITIONNUM) */
    val index2 = index("PE_POSITION_POSITIONNUM", positionnum :: HNil)
    /** Index over (subcompanyid) (database name PE_POSITION_SUBCOMPANYID) */
    val index3 = index("PE_POSITION_SUBCOMPANYID", subcompanyid :: HNil)
    /** Index over (workplaceid) (database name PE_POSITION_WORKPLACEID) */
    val index4 = index("PE_POSITION_WORKPLACEID", workplaceid :: HNil)
  }
