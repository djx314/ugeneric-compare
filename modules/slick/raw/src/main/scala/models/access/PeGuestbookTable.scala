...
trait PeGuestbookTable {
  ...
  case class PeGuestbookRow(guestid: Int = 0, kindid: Option[Int] = Some(0), topicid: Option[Int] = Some(0), guestmaxid: Option[Int], guesttype: Option[Int], guestname: Option[String], guestsex: Option[String], guestoicq: Option[String], guestmsn: Option[String], guesticq: Option[String], guestemail: Option[String], guesthomepage: Option[String], guestimages: Option[String], guestface: Option[String], guestip: Option[String], guesttitle: Option[String], guestcontent: Option[String], guestdatetime: Option[java.sql.Timestamp], hits: Option[Int] = Some(0), replynum: Option[Int] = Some(0), lastreplycontent: Option[String], lastreplyguest: Option[String], lastreplytime: Option[java.sql.Timestamp], lastreplytitle: Option[String], guestreply: Option[String], guestreplyadmin: Option[String], guestreplydatetime: Option[java.sql.Timestamp], quintessence: Option[Int] = Some(0), guestispassed: Boolean, guestisprivate: Boolean, ontop: Option[Int] = Some(0), replyisprivate: Boolean, guestcontentlength: Option[Int] = Some(0))
  ...
  class PeGuestbook(_tableTag: Tag) extends profile.api.Table[PeGuestbookRow](_tableTag, "PE_GuestBook") {
    def * = (guestid :: kindid :: topicid :: guestmaxid :: guesttype :: guestname :: guestsex :: guestoicq :: guestmsn :: guesticq :: guestemail :: guesthomepage :: guestimages :: guestface :: guestip :: guesttitle :: guestcontent :: guestdatetime :: hits :: replynum :: lastreplycontent :: lastreplyguest :: lastreplytime :: lastreplytitle :: guestreply :: guestreplyadmin :: guestreplydatetime :: quintessence :: guestispassed :: guestisprivate :: ontop :: replyisprivate :: guestcontentlength :: HNil).mapTo[PeGuestbookRow]
    ...
    val index1 = index("PE_GUESTBOOK_GUESTISPASSED", guestispassed :: HNil)
    /** Index over (guestmaxid) (database name PE_GUESTBOOK_GUESTMAXID) */
    val index2 = index("PE_GUESTBOOK_GUESTMAXID", guestmaxid :: HNil)
    /** Index over (guesttype) (database name PE_GUESTBOOK_GUESTTYPE) */
    val index3 = index("PE_GUESTBOOK_GUESTTYPE", guesttype :: HNil)
    /** Index over (kindid) (database name PE_GUESTBOOK_KINDID) */
    val index4 = index("PE_GUESTBOOK_KINDID", kindid :: HNil)
    /** Index over (ontop) (database name PE_GUESTBOOK_ONTOP) */
    val index5 = index("PE_GUESTBOOK_ONTOP", ontop :: HNil)
    /** Index over (quintessence) (database name PE_GUESTBOOK_QUINTESSENCE) */
    val index6 = index("PE_GUESTBOOK_QUINTESSENCE", quintessence :: HNil)
    /** Index over (replynum) (database name PE_GUESTBOOK_REPLYNUM) */
    val index7 = index("PE_GUESTBOOK_REPLYNUM", replynum :: HNil)
    /** Index over (topicid) (database name PE_GUESTBOOK_TOPICID) */
    val index8 = index("PE_GUESTBOOK_TOPICID", topicid :: HNil)
  }
