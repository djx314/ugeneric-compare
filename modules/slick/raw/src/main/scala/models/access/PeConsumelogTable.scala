...
trait PeConsumelogTable {
  ...
  case class PeConsumelogRow(logid: Int, username: Option[String], moduletype: Option[Int], infoid: Option[Int] = Some(0), point: Option[scala.math.BigDecimal] = Some(scala.math.BigDecimal("0")), logtime: Option[java.sql.Timestamp], times: Option[Int] = Some(0), incomePayout: Option[Int] = Some(0), remark: Option[String], ip: Option[String], inputer: Option[String])
  ...
  class PeConsumelog(_tableTag: Tag) extends profile.api.Table[PeConsumelogRow](_tableTag, "PE_ConsumeLog") {
    def * = (logid :: username :: moduletype :: infoid :: point :: logtime :: times :: incomePayout :: remark :: ip :: inputer :: HNil).mapTo[PeConsumelogRow]
    ...
    val index1 = index("PE_CONSUMELOG_INCOME_PAYOUT", incomePayout :: HNil)
    /** Index over (infoid) (database name PE_CONSUMELOG_INFOID) */
    val index2 = index("PE_CONSUMELOG_INFOID", infoid :: HNil)
    /** Index over (logtime) (database name PE_CONSUMELOG_LOGTIME) */
    val index3 = index("PE_CONSUMELOG_LOGTIME", logtime :: HNil)
    /** Index over (moduletype) (database name PE_CONSUMELOG_MODULETYPE) */
    val index4 = index("PE_CONSUMELOG_MODULETYPE", moduletype :: HNil)
    /** Index over (username) (database name PE_CONSUMELOG_USERNAME) */
    val index5 = index("PE_CONSUMELOG_USERNAME", username :: HNil)
  }
