...
trait PeLabelTable {
  ...
  case class PeLabelRow(
    labelid: Int,
    labelname: Option[String],
    labelclass: Option[String],
    labeltype: Option[Short] = Some(0),
    pagenum: Option[Int] = Some(0),
    reflashtime: Option[Short] = Some(0),
    labelintro: Option[String],
    priority: Option[Int] = Some(0),
    labelcontent: Option[String],
    areacollectionid: Option[Int] = Some(0),
    fieldlist: Option[String]
  )
  ...
  class PeLabel(_tableTag: Tag) extends profile.api.Table[PeLabelRow](_tableTag, "PE_Label") {
    def * =
      (labelid :: labelname :: labelclass :: labeltype :: pagenum :: reflashtime :: labelintro :: priority :: labelcontent :: areacollectionid :: fieldlist :: HNil)
        .mapTo[PeLabelRow]
    ...
    val index1 = index("PE_LABEL_AREACOLLECTIONID", areacollectionid :: HNil)

    /** Index over (pagenum) (database name PE_LABEL_PAGENUM) */
    val index2 = index("PE_LABEL_PAGENUM", pagenum :: HNil)

    /** Index over (reflashtime) (database name PE_LABEL_PAGENUM1) */
    val index3 = index("PE_LABEL_PAGENUM1", reflashtime :: HNil)
  }
