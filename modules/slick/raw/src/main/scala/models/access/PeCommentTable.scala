...
trait PeCommentTable {
  ...
  case class PeCommentRow(
    ...
    replyname: Option[String],
    replycontent: Option[String],
    replytime: Option[java.sql.Timestamp],
    passed: Boolean,
    support: Option[Int],
    opposed: Option[Int]
  )
  ...
  class PeComment(_tableTag: Tag) extends profile.api.Table[PeCommentRow](_tableTag, "PE_Comment") {
    def * =
      (commentid :: moduletype :: infoid :: usertype :: username :: sex :: oicq :: msn :: icq :: email :: homepage :: ip :: writetime :: score :: content :: replyname :: replycontent :: replytime :: passed :: support :: opposed :: HNil)
        .mapTo[PeCommentRow]
    ...
    val index1 = index("PE_COMMENT_INFOID", infoid :: HNil)

    /** Index over (moduletype) (database name PE_COMMENT_MODULETYPE) */
    val index2 = index("PE_COMMENT_MODULETYPE", moduletype :: HNil)

    /** Index over (passed) (database name PE_COMMENT_PASSED) */
    val index3 = index("PE_COMMENT_PASSED", passed :: HNil)
  }
