...
trait PeDownserverTable {
  ...
  case class PeDownserverRow(serverid: Int, channelid: Option[Int] = Some(0), servername: Option[String], serverurl: Option[String], serverlogo: Option[String], orderid: Option[Int] = Some(0), showtype: Option[Int] = Some(0), infopoint: Option[Int], infopurview: Option[Int] = Some(0), arrgroupid: Option[String], chargetype: Option[Int] = Some(0), pitchtime: Option[Int] = Some(0), readtimes: Option[Int] = Some(0), dividepercent: Option[Int] = Some(0))
  ...
  class PeDownserver(_tableTag: Tag) extends profile.api.Table[PeDownserverRow](_tableTag, "PE_DownServer") {
    def * = (serverid :: channelid :: servername :: serverurl :: serverlogo :: orderid :: showtype :: infopoint :: infopurview :: arrgroupid :: chargetype :: pitchtime :: readtimes :: dividepercent :: HNil).mapTo[PeDownserverRow]
    ...
    val index1 = index("PE_DOWNSERVER_ARRGROUPID", arrgroupid :: HNil)
    /** Index over (channelid) (database name PE_DOWNSERVER_CHANNELID) */
    val index2 = index("PE_DOWNSERVER_CHANNELID", channelid :: HNil)
    /** Index over (orderid) (database name PE_DOWNSERVER_ORDERID) */
    val index3 = index("PE_DOWNSERVER_ORDERID", orderid :: HNil)
  }
