...
trait PeInvoiceitemTable {
  ...
  class PeInvoiceitem(_tableTag: Tag) extends profile.api.Table[PeInvoiceitemRow](_tableTag, "PE_InvoiceItem") {
    def * =
      (invoiceid :: clientid :: username :: orderformid :: invoicetype :: invoicenum :: invoicetitle :: invoicecontent :: invoicedate :: totalmoney :: drawer :: inputer :: inputtime :: HNil)
        .mapTo[PeInvoiceitemRow]
    ...
    val index1 = index("PE_INVOICEITEM_CLIENTID", clientid :: HNil)

    /** Index over (invoicenum) (database name PE_INVOICEITEM_INVOICENUM) */
    val index2 = index("PE_INVOICEITEM_INVOICENUM", invoicenum :: HNil)

    /** Index over (orderformid) (database name PE_INVOICEITEM_ORDERFORMID) */
    val index3 = index("PE_INVOICEITEM_ORDERFORMID", orderformid :: HNil)

    /** Index over (username) (database name PE_INVOICEITEM_USERNAME) */
    val index4 = index("PE_INVOICEITEM_USERNAME", username :: HNil)
  }
