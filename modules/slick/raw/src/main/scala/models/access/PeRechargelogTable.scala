...
trait PeRechargelogTable {
  ...
  class PeRechargelog(_tableTag: Tag) extends profile.api.Table[PeRechargelogRow](_tableTag, "PE_RechargeLog") {
    def * = (logid :: username :: validnum :: validunit :: incomePayout :: logtime :: remark :: ip :: inputer :: HNil).mapTo[PeRechargelogRow]
    ...
    val index1 = index("PE_RECHARGELOG_USERNAME", username :: HNil)

    /** Index over (validnum) (database name PE_RECHARGELOG_VALIDNUM) */
    val index2 = index("PE_RECHARGELOG_VALIDNUM", validnum :: HNil)
  }
