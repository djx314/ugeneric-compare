...
trait PeSupplyTable {
  ...
  case class PeSupplyRow(supplyid: Int, channelid: Int, classid: Int, specialid: Option[Int], supplyname: Option[String], supplytype: Option[Int], supplytitle: String, supplyintro: Option[String], supplyperiod: Option[Int], tradetype: Option[Int], priceintro: Option[String], supplypicurl: Option[String], templateid: Option[Int], skinid: Option[Int], status: Option[Int], username: String, hits: Option[Int], updatetime: Option[java.sql.Timestamp], deleted: Option[Boolean], commandtype: Option[Int], commandchanneldays: Option[Int], commandclassdays: Option[Int])
  ...
  class PeSupply(_tableTag: Tag) extends profile.api.Table[PeSupplyRow](_tableTag, "PE_Supply") {
    def * = (supplyid :: channelid :: classid :: specialid :: supplyname :: supplytype :: supplytitle :: supplyintro :: supplyperiod :: tradetype :: priceintro :: supplypicurl :: templateid :: skinid :: status :: username :: hits :: updatetime :: deleted :: commandtype :: commandchanneldays :: commandclassdays :: HNil).mapTo[PeSupplyRow]
    ...
    val index1 = index("PE_SUPPLY_CHANNELID", channelid :: HNil)
    /** Index over (classid) (database name PE_SUPPLY_CLASSID) */
    val index2 = index("PE_SUPPLY_CLASSID", classid :: HNil)
    /** Index over (skinid) (database name PE_SUPPLY_SKINID) */
    val index3 = index("PE_SUPPLY_SKINID", skinid :: HNil)
    /** Index over (specialid) (database name PE_SUPPLY_SPECIALID) */
    val index4 = index("PE_SUPPLY_SPECIALID", specialid :: HNil)
    /** Index over (templateid) (database name PE_SUPPLY_TEMPLATEID) */
    val index5 = index("PE_SUPPLY_TEMPLATEID", templateid :: HNil)
  }
