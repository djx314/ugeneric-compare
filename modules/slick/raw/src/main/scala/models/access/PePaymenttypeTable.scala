...
trait PePaymenttypeTable {
  ...
  case class PePaymenttypeRow(typeid: Int = 0, typename: Option[String], intro: Option[String], discount: Option[Double] = Some(0.0), isdefault: Option[Boolean], orderid: Option[Int] = Some(0), isdisabled: Option[Boolean])
  ...
  class PePaymenttype(_tableTag: Tag) extends profile.api.Table[PePaymenttypeRow](_tableTag, "PE_PaymentType") {
    def * = (typeid :: typename :: intro :: discount :: isdefault :: orderid :: isdisabled :: HNil).mapTo[PePaymenttypeRow]
    ...
    val index1 = index("PE_PAYMENTTYPE_ORDERID", orderid :: HNil)
  }
