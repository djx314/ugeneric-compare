...
trait PeBankrollitemTable {
  ...
  case class PeBankrollitemRow(
    ...
    moneytype: Option[Int] = Some(0),
    currencytype: Option[Int] = Some(0),
    ebankid: Option[Int] = Some(0),
    bank: Option[String],
    incomePayout: Option[Int] = Some(0),
    orderformid: Option[Int] = Some(0),
    paymentid: Option[Int] = Some(0),
  ...
  class PeBankrollitem(_tableTag: Tag) extends profile.api.Table[PeBankrollitemRow](_tableTag, "PE_BankrollItem") {
    def * =
      (itemid :: username :: clientid :: dateandtime :: money :: moneytype :: currencytype :: ebankid :: bank :: incomePayout :: orderformid :: paymentid :: remark :: logtime :: inputer :: ip :: HNil)
        .mapTo[PeBankrollitemRow]
    ...
    val index1 = index("PE_BANKROLLITEM_CLIENTID", clientid :: HNil)

    /** Index over (ebankid) (database name PE_BANKROLLITEM_EBANKID) */
    val index2 = index("PE_BANKROLLITEM_EBANKID", ebankid :: HNil)

    /** Index over (orderformid) (database name PE_BANKROLLITEM_ORDERFORMID) */
    val index3 = index("PE_BANKROLLITEM_ORDERFORMID", orderformid :: HNil)

    /** Index over (paymentid) (database name PE_BANKROLLITEM_PAYMENTID) */
    val index4 = index("PE_BANKROLLITEM_PAYMENTID", paymentid :: HNil)

    /** Index over (username) (database name PE_BANKROLLITEM_USERNAME) */
    val index5 = index("PE_BANKROLLITEM_USERNAME", username :: HNil)
  }
