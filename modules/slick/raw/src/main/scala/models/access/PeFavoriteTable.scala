...
trait PeFavoriteTable {
  ...
  class PeFavorite(_tableTag: Tag) extends profile.api.Table[PeFavoriteRow](_tableTag, "PE_Favorite") {
    def * = (id :: userid :: channelid :: infoid :: dateandtime :: HNil).mapTo[PeFavoriteRow]
    ...
    val index1 = index("PE_FAVORITE_CHANNELID", channelid :: HNil)

    /** Index over (infoid) (database name PE_FAVORITE_INFOID) */
    val index2 = index("PE_FAVORITE_INFOID", infoid :: HNil)

    /** Index over (userid) (database name PE_FAVORITE_USERID) */
    val index3 = index("PE_FAVORITE_USERID", userid :: HNil)
  }
