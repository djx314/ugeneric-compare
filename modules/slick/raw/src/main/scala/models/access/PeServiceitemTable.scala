...
trait PeServiceitemTable {
  ...
  case class PeServiceitemRow(
    itemid: Int,
    clientid: Option[Int] = Some(0),
    contacterid: Option[Int] = Some(0),
    orderformid: Option[Int] = Some(0),
    servicetime: Option[java.sql.Timestamp],
    servicetype: Option[String],
    servicemode: Option[String],
    servicetitle: Option[String],
    servicecontent: Option[String],
    serviceresult: Option[Int] = Some(0),
    taketime: Option[Int] = Some(0),
  ...
  class PeServiceitem(_tableTag: Tag) extends profile.api.Table[PeServiceitemRow](_tableTag, "PE_ServiceItem") {
    def * =
      (itemid :: clientid :: contacterid :: orderformid :: servicetime :: servicetype :: servicemode :: servicetitle :: servicecontent :: serviceresult :: taketime :: processor :: inputer :: feedback :: confirmtime :: confirmcaller :: confirmscore :: confirmfeedback :: remark :: HNil)
        .mapTo[PeServiceitemRow]
    ...
    val index1 = index("PE_SERVICEITEM_CLIENTID", clientid :: HNil)

    /** Index over (contacterid) (database name PE_SERVICEITEM_CONTACTERID) */
    val index2 = index("PE_SERVICEITEM_CONTACTERID", contacterid :: HNil)

    /** Index over (orderformid) (database name PE_SERVICEITEM_ORDERFORMID) */
    val index3 = index("PE_SERVICEITEM_ORDERFORMID", orderformid :: HNil)
  }
