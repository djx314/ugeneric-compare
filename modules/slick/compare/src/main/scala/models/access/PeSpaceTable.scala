...
trait PeSpaceTable {
  ...
  case class PeSpaceRow(id: Int, userid: Option[Int] = Some(0), classid: Option[Int] = Some(0), name: String, birthday: Option[java.sql.Timestamp], photo: Option[String], intro: Option[String], address: Option[String], tel: Option[String], fax: Option[String], company: Option[String], department: Option[String], zipcode: Option[String], homepage: Option[String], email: Option[String], qq: Option[Int] = Some(0), `type`: Option[Short] = Some(0), lastusetime: Option[java.sql.Timestamp], ontop: Option[Boolean], passed: Option[Boolean], iselite: Option[Boolean], hits: Option[Int] = Some(0), orderid: Option[Int] = Some(0), linkurl: Option[String], listnum: Option[Short] = Some(10))
  ...
  class PeSpace(_tableTag: Tag) extends profile.api.Table[PeSpaceRow](_tableTag, "PE_Space") {
    ...
    val index1 = index("PE_SPACE_CLASSID", classid)
    /** Index over (linkurl) (database name PE_SPACE_LINKURL) */
    val index2 = index("PE_SPACE_LINKURL", linkurl)
    /** Index over (listnum) (database name PE_SPACE_LISTNUM) */
    val index3 = index("PE_SPACE_LISTNUM", listnum)
    /** Index over (orderid) (database name PE_SPACE_ORDERID) */
    val index4 = index("PE_SPACE_ORDERID", orderid)
    /** Index over (userid) (database name PE_SPACE_USERID) */
    val index5 = index("PE_SPACE_USERID", userid)
    /** Index over (zipcode) (database name PE_SPACE_ZIPCODE) */
    val index6 = index("PE_SPACE_ZIPCODE", zipcode)
  }
