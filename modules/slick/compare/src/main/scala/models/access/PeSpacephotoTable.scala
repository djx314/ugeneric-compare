...
trait PeSpacephotoTable {
  ...
  case class PeSpacephotoRow(id: Int = 0, userid: Int = 0, blogid: Option[Int] = Some(0), title: String, content: Option[String], datetime: java.sql.Timestamp, hits: Option[Int] = Some(0), plnum: Option[Int] = Some(0))
  ...
  class PeSpacephoto(_tableTag: Tag) extends profile.api.Table[PeSpacephotoRow](_tableTag, "PE_SpacePhoto") {
    ...
    val index1 = index("PE_SPACEPHOTO_PLNUM", plnum)
    /** Index over (userid) (database name PE_SPACEPHOTO_USERID) */
    val index2 = index("PE_SPACEPHOTO_USERID", userid)
    /** Index over (blogid) (database name PE_SPACEPHOTO_USERID1) */
    val index3 = index("PE_SPACEPHOTO_USERID1", blogid)
  }
