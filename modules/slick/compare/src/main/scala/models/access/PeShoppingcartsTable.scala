...
trait PeShoppingcartsTable {
  ...
  case class PeShoppingcartsRow(
    cartitemid: Int,
    cartid: Option[String],
    productid: Option[Int] = Some(0),
    quantity: Option[Int] = Some(0),
    presentid: Option[Int] = Some(0),
  ...
  class PeShoppingcarts(_tableTag: Tag) extends profile.api.Table[PeShoppingcartsRow](_tableTag, "PE_ShoppingCarts") {
    ...
    val index1 = index("PE_SHOPPINGCARTS_CARTID", cartid)

    /** Index over (presentid) (database name PE_SHOPPINGCARTS_PRESENTID) */
    val index2 = index("PE_SHOPPINGCARTS_PRESENTID", presentid)

    /** Index over (productid) (database name PE_SHOPPINGCARTS_PRODUCTID) */
    val index3 = index("PE_SHOPPINGCARTS_PRODUCTID", productid)
  }
