...
trait PeSpecialTable {
  ...
  case class PeSpecialRow(specialid: Int, channelid: Int, specialname: String, opentype: Int, specialpicurl: Option[String], orderid: Int, tips: Option[String], readme: Option[String], skinid: Int, templateid: Int, specialdir: String, maxperpage: Option[Int], iselite: Option[Boolean], customContent: Option[String])
  ...
  class PeSpecial(_tableTag: Tag) extends profile.api.Table[PeSpecialRow](_tableTag, "PE_Special") {
    ...
    val index1 = index("PE_SPECIAL_CHANNELID", channelid)
    /** Index over (orderid) (database name PE_SPECIAL_ORDERID) */
    val index2 = index("PE_SPECIAL_ORDERID", orderid)
    /** Index over (skinid) (database name PE_SPECIAL_SKINID) */
    val index3 = index("PE_SPECIAL_SKINID", skinid)
    /** Index over (templateid) (database name PE_SPECIAL_TEMPLATEID) */
    val index4 = index("PE_SPECIAL_TEMPLATEID", templateid)
  }
