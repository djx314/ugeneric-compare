...
trait PeKeylinkTable {
  ...
  case class PeKeylinkRow(
    id: Int,
    channelid: Option[Short],
    source: Option[String],
    replacetext: Option[String],
    isuse: Option[Short] = Some(1),
    linktype: Option[Short] = Some(0),
    opentype: Option[Short] = Some(0),
    replacetype: Option[Short] = Some(0),
    priority: Option[Short] = Some(1)
  )
  ...
  class PeKeylink(_tableTag: Tag) extends profile.api.Table[PeKeylinkRow](_tableTag, "PE_KeyLink") {
    ...
    val index1 = index("PE_KEYLINK_CHANNELID", channelid)

    /** Index over (replacetype) (database name PE_KEYLINK_REPLACENUM) */
    val index2 = index("PE_KEYLINK_REPLACENUM", replacetype)
  }
