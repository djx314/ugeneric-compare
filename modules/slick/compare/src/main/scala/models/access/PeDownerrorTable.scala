...
trait PeDownerrorTable {
  ...
  case class PeDownerrorRow(errorid: Int = 0, channelid: Option[Int] = Some(0), infoid: Option[Int] = Some(0), urlid: Option[Int] = Some(0), errortimes: Option[Int] = Some(0))
  ...
  class PeDownerror(_tableTag: Tag) extends profile.api.Table[PeDownerrorRow](_tableTag, "PE_DownError") {
    ...
    val index1 = index("PE_DOWNERROR_CHANNELID", channelid)
    /** Index over (infoid) (database name PE_DOWNERROR_INFOID) */
    val index2 = index("PE_DOWNERROR_INFOID", infoid)
    /** Index over (urlid) (database name PE_DOWNERROR_URLID) */
    val index3 = index("PE_DOWNERROR_URLID", urlid)
  }
