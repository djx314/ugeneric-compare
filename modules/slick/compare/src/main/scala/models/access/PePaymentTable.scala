...
trait PePaymentTable {
  ...
  case class PePaymentRow(paymentid: Int = 0, username: Option[String], orderformid: Option[Int] = Some(0), paymentnum: Option[String], ebankid: Option[Int] = Some(0), moneypay: Option[scala.math.BigDecimal] = Some(scala.math.BigDecimal("0")), moneytrue: Option[scala.math.BigDecimal] = Some(scala.math.BigDecimal("0")), paytime: Option[java.sql.Timestamp], status: Option[Int] = Some(0), ebankinfo: Option[String], remark: Option[String])
  ...
  class PePayment(_tableTag: Tag) extends profile.api.Table[PePaymentRow](_tableTag, "PE_Payment") {
    ...
    val index1 = index("PE_PAYMENT_EBANKID", ebankid)
    /** Index over (moneypay) (database name PE_PAYMENT_MONEYPAY) */
    val index2 = index("PE_PAYMENT_MONEYPAY", moneypay)
    /** Index over (orderformid) (database name PE_PAYMENT_ORDERFORMID) */
    val index3 = index("PE_PAYMENT_ORDERFORMID", orderformid)
    /** Index over (paymentnum) (database name PE_PAYMENT_PAYMENTNUM) */
    val index4 = index("PE_PAYMENT_PAYMENTNUM", paymentnum)
    /** Index over (username) (database name PE_PAYMENT_USERNAME) */
    val index5 = index("PE_PAYMENT_USERNAME", username)
  }
